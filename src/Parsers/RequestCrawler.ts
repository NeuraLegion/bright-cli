...
export class RequestCrawler extends Transform {
  ...
  constructor({pool = 250, timeout = 5000, maxRedirects = 20}: {
    timeout: number,
    pool?: number,
    maxRedirects?: number
  }) {
  ...
  public _transform(data: Options | Options[], encoding: string, done: TransformCallback): void {
    const requests: Options[] = Array.isArray(data) ?
      data :
      [data];

    this.splitByChunks<Options[], Options>(requests, this.pool)
      .reduce((total: Promise<void>, partOfRequests: Options[]) =>
          total.then(() =>
            Promise.all<void>(
  ...
  private executeRequest(opt: Options): Promise<void> {
    return new Promise<void>((resolve) => this.proxy
        .start(opt)
        .once('end', resolve)
  ...
  private splitByChunks<T extends R[], R>(
    array: T,
    count: number
  ): R[][] {
