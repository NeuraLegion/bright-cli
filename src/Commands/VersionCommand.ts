...
export class VersionCommand implements yargs.CommandModule {
  ...
  public async handler(): Promise<void> {
    ...
    const localMatches: RegExpMatchArray | null = localNpmList.match(/ @nexploit\/cli@(.*)\n/);
    const localNpmVersion: string = (localMatches && localMatches[1] ? localMatches[1] : '')
      .replace(/"invalid"/gi, '')
      .trim();

    const globalNpmList: string = await VersionCommand.executeCommand('npm list -g --depth=0');
    const globalMatches: RegExpMatchArray | null = globalNpmList.match(/ @nexploit\/cli@(.*)\n/);
    const globalNpmVersion: string = (globalMatches && globalMatches[1] ? globalMatches[1] : '')
      .replace(/"invalid"/gi, '')
      .trim();
    ...
    if (localNpmVersion && globalNpmVersion && localNpmVersion !== globalNpmVersion) {
      console.log('To avoid issues with CLI please make sure your global and local NexPloit versions match, ' +
